...
type NavChild = { label: string; href: string; icon: React.ReactNode; hideForAdmin?: boolean; hideForPmFornitore?: boolean; };
type NavItem  = { label: string; href?: string; icon: React.ReactNode; hideForAdmin?: boolean; hideForPmFornitore?: boolean; children?: NavChild[]; };

export default function Topbar() {
  const pathname = usePathname();
  const router   = useRouter();
  const { user, isAdmin, isPmFornitore } = useUser();
  ...
  const shouldHide = (item: { hideForAdmin?: boolean; hideForPmFornitore?: boolean }) =>
    (isAdmin && item.hideForAdmin) || (isPmFornitore && item.hideForPmFornitore);
  ...
  return (
    <header style={{ height: 56, backgroundColor: "#0f172a", display: "flex", alignItems: "center", padding: "0 24px", gap: 0, flexShrink: 0, borderBottom: "1px solid #1e293b" }}>
        ...
        {nav.map((item) => {
            ...
            return (
              <div key={item.label} style={{ position: "relative" }}>
                <button onClick={() => setOpenDropdown(open ? null : item.label)}
                  style={{ display: "flex", alignItems: "center", gap: 7, padding: "6px 12px", borderRadius: 7, border: "none", background: active ? "rgba(255,255,255,0.1)" : "transparent", cursor: "pointer", fontFamily: "inherit", fontSize: 13, fontWeight: active ? 600 : 500, color: active ? "#fff" : "#94a3b8", transition: "all 0.15s" }}
                ...
                {open && (
                  <div className="animate-fadeIn" style={{ position: "absolute", top: "calc(100% + 8px)", left: 0, width: 200, backgroundColor: "white", border: "1px solid var(--border)", borderRadius: 10, boxShadow: "0 8px 24px rgba(0,0,0,0.12)", overflow: "hidden", zIndex: 50 }}>
                    {visibleChildren.map((child) => {
                      const childActive = pathname.startsWith(child.href);
                      return (
                        <Link key={child.href} href={child.href} onClick={() => setOpenDropdown(null)}
                          style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", textDecoration: "none", backgroundColor: childActive ? "#eff6ff" : "transparent", fontSize: 13, fontWeight: childActive ? 600 : 400, color: childActive ? "#2563eb" : "#374151", borderBottom: "1px solid #f1f5f9", transition: "background 0.1s" }}
          ...
          return (
            <Link key={item.href} href={item.href!}
              style={{ display: "flex", alignItems: "center", gap: 7, padding: "6px 12px", borderRadius: 7, textDecoration: "none", backgroundColor: active ? "rgba(255,255,255,0.1)" : "transparent", fontSize: 13, fontWeight: active ? 600 : 500, color: active ? "#fff" : "#94a3b8", transition: "all 0.15s" }}
      ...
      <div ref={userRef} style={{ position: "relative", flexShrink: 0 }}>
        <button onClick={() => setOpenUser(!openUser)}
          ...
          <div style={{ width: 28, height: 28, borderRadius: "50%", flexShrink: 0, backgroundColor: user?.avatar_bg ?? "#3b82f6", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: user?.avatar_colore ?? "white", letterSpacing: "0.02em" }}>
